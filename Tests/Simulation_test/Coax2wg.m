%
% Adaptador Conector N a Guia de onda WR-90 -> Keysight/Agilent x281A
%
close all
clear
clc
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Simulacion %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
physical_constants;
unit            = 1e-3;             % unidades en milimetros
NumTS           = 30000;            % max. number of timesteps

% Modo de transmision TE10
m = 1;
n = 0;
...
ProbeDepth      = 5.1;%b/2+0.5;              % Probe insertion depth inside waveguide  ?????
...
mesh.z = SmoothMeshLines([0 mesh.z ], mesh_res(3),1.3);%clearance+length/2+WallThickness length+WallThickness+2*clearance length+2*WallThickness+2*clearance], mesh_res(3),1.3);

CSX = DefineRectGrid(CSX, unit,mesh);


%%%%%   Construyo la guia de onda   %%%%%
...
CSX = AddCylinder(CSX,'air',20,[a/2+WallThickness b+WallThickness WallThickness+BackShort], [a/2+WallThickness b+2*WallThickness WallThickness+BackShort],OuterCondN/2,'Transform',{'Translate', [clearance+cl_x,clearance+cl_y,clearance]});
...
start = clearance+[cl_x+a/2+WallThickness,cl_y+b+2*WallThickness+NLength,+BackShort+WallThickness];
stop  = clearance+[cl_x+a/2+WallThickness,cl_y+b+2*WallThickness,+BackShort+WallThickness];  
% More info of AddCoaxialPort here:
%                       https://github.com/thliebig/openEMS/blob/master/matlab/AddCoaxialPort.m
